...
input_csv_path = "../../data/processed/votes_SentimentAnalysis_02.csv"
output_csv_path = "../../data/processed/votes_SentimentAnalysis_03.csv"
output_json_path = "../../data/processed/votes_SentimentAnalysis_03.json"
...
def process_row(row):
    if row["reason"]:
        ...
        row["reason_formatted_dv2"] = corrected_text
        ...
        if len(corrected_text) > token_limit:
            ...
            for chunk in chunks:
                ...
                try:
                    row["Sentiment Intensity_dv2"] = float(response.choices[0].text.strip().split()[0])
                except ValueError:
                    print(f"Error: Unable to convert {response.choices[0].text.strip().split()[0]} to a float.")
                    row["Sentiment Intensity_dv2"] = None
                row["Intensity Tokens_dv2"] = response.usage["total_tokens"]
                row["Intensity Cost_dv2"] = response.usage["total_tokens"] * cost_per_token
        else:
            response = call_openai_api(corrected_text)
            try:
                row["Sentiment Intensity_dv2"] = float(response.choices[0].text.strip().split()[0])
            except ValueError:
                print(f"Error: Unable to convert {response.choices[0].text.strip().split()[0]} to a float.")
